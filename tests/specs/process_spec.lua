...
describe("process execution", function()
  it("basic command", function()
    local result = process.new({ cmd = { "cat", "process_test" }, cwd = util.get_test_files_dir() }):spawn_blocking(1000)
    assert(result)
  ...
  it("can cat a file", function()
    local result = process.new({ cmd = { "cat", "a.txt" }, cwd = util.get_test_files_dir() }):spawn_blocking(1000)
  ...
  it("basic command trim", function()
    local result =
      process.new({ cmd = { "cat", "process_test" }, cwd = util.get_test_files_dir() }):spawn_blocking(1000):trim()
    assert(result)
    assert.are.same(result.stdout, {
      "This is a test file",
      "It is intended to be read by cat and returned to neovim using the process api",
